...
Note you must use __Ruby MRI__ to run mrbcc.rb, not mruby!

= Example

Example to run array test from mruby:
...
If you make too long .rb input it will probably crash, still investigating.
...
Here is a list of all the tests from mruby (but cannot run all at once = crash):
